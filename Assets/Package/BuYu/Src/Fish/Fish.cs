    ...
    public class Fish : IGlobalEffect
    {
        ...
        public void SetPackage(byte packageType)
        {
            ...
            if (FishConfig.Instance.m_ChestInfo.ChestMap.TryGetValue(packageType, out pChest))
            {
                ...
                else//是卡片
                {
                    ...
                    m_FishCard.transform.GetChild(0).GetComponent<Image>().name = pReward.RewardItemVec[0].ItemID.ToString();
                    if (m_FishType == 9)
                    {
        ...
        public bool Update(float delta)
        {
            if (IsBossFish() && m_bgsoundDelay > 0)
            {
                ...
                if (m_bgsoundDelay < 0)
                {
                    //AudioManager.Instance.PlayerBGMusic(Audio.EffectBGType.EffectBGSound3 + m_FishID % 2);
                }
            }
            if (m_Delay)
            {
        ...
        {
            //SceneRuntime.SceneModel.ChestMgr.MoveBox(SceneRuntime.WorldToNGUI(m_BoxModel.transform.position));
        }
        ...
        public void Destroy()
        {
            ...
            if (m_FishCard != null)
            {
                if (m_CatchSeat == SceneRuntime.PlayerMgr.MyClientSeat)
                {
                    // SceneRuntime.SceneModel.LogicUI.GetSkillBar.JudgeGuide();
                    Vector2 vecPos = SceneRuntime.SceneModel.PlayerMgr.MySelf.Launcher.LauncherPos;
                   // SceneRuntime.SceneModel.LogicUI.MoveCard(m_FishCard, new Vector3(vecPos.x, vecPos.y, 0));
                }
                else
                {
