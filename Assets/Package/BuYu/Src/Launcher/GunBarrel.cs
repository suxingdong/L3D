...
namespace BuYu
{
    ...
    {
        private GameObject m_HandleObj;
        private List<Transform> m_BarrelTransList = new List<Transform>();
        private List<Animator> m_AnimatorList = new List<Animator>();
        private GameObject[] m_BarrelLightdot = new GameObject[3];
        private GameObject m_MuzzleEft = null; //冰炮炮口冰球特效
        private Transform m_BaseTransform;
        private Transform m_GaiTransfom; //盖子变换
        private Transform m_MuzzleEftTrans; //冰炮炮口冰球特效
        private Text m_BulletConsumeLabel; //子弹基础消耗
        private Vector2 m_GunPivot = new Vector2(); //炮管中心轴点的位置
        private float m_LightDotDelay;
        private float m_LightDotInterval;
        private int m_LightDotFlag;
        private float m_AnimTimes;
        private float m_AnimInterval; //动画播放频率
        private GunBarrelDate m_GunBarrelDate;
        private MuzzleEffectAnim m_MuzzleEftAnim;
        private bool m_IsPlayAnim; //
        private int m_LightCount;
        private ushort m_BulletConsume; //炮的发出子弹的基础伤害
        private byte m_Seat;
        ...
        public void Init()
        {
            ...
            m_BulletConsumeLabel = m_BaseTransform.GetChild(indx).GetChild(0).gameObject.GetComponent<Text>();
        ...
        private void StandbyAnimation(float deltaTime)
        {
            if (m_GunBarrelDate.LightDot)
        ...
        private void MoveBallEffect(float deltaTime)
        {
        ...
        private void LightDotFlicker(float deltaTime)
        {
        ...
        private void ShowActive(bool active)
        {
            //if (SceneRuntime.MaskEffect)
            //{
            if (m_GunBarrelDate.MuzzleEft && m_MuzzleEft != null)
